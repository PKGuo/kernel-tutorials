...
DEFAULT_CROSS_VALIDATION_KWARGS = {
    "train_test_split": True, "train_ratio": 0.5, "seed": 0x5F3759DF, "nb_folds": 2}
...
def compute_reconstruction_matrix(features, features_dash, nb_folds):
    ...
    def fold_test_error(regularizer):
        ...
        for i in range(nb_folds):
            W = np.linalg.lstsq(
                features_folds[i], features_dash_folds[i], rcond=regularizer)[0]
            test_fold_idx = list(range(nb_folds))
            test_fold_idx.remove(i)
            features_test = np.concatenate(
                [features_folds[j] for j in test_fold_idx], axis=0)
            features_dash_test = np.concatenate(
                [features_dash_folds[j] for j in test_fold_idx], axis=0)
...
def compute_pointwise_lfre(features, features_dash, nb_local_envs, cross_validation_kwargs=DEFAULT_CROSS_VALIDATION_KWARGS):
    ...
    features = standardize_features(features, train_idx)
    features_dash = standardize_features(features_dash, train_idx)
    features_train, features_dash_train, features_test, features_dash_test = features[
        train_idx], features_dash[train_idx], features, features_dash

    n_test = features_test.shape[0]
    squared_dist = np.sum(features_train**2, axis=1) + np.sum(features_test **
